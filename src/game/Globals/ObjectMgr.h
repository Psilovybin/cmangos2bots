...
{
    uint32_t spellId;
    uint32_t questStart;                                      // quest start (quest must be active or rewarded for spell apply)
    uint32_t questEnd;                                        // quest end (quest don't must be rewarded for spell apply)
    bool questStartCanActive;                             // if true then quest start can be active (not only rewarded)
    uint8_t castFlags;
    uint16_t conditionId;                                     // intends to replace questStart, questEnd, questStartCanActive
